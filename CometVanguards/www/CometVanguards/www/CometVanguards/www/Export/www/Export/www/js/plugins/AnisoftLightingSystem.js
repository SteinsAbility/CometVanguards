...
Imported.AnisoftAdvancedLighting = '0.9.0'
//=============================================================================
// AnisoftLightingSystem.js
//=============================================================================

var Anisoft = Anisoft || {};
Anisoft.ALS = Anisoft.ALS || {};


/*:
* @plugindesc <AnisoftLightingSystem>
* @author Anisoft/Quasi (QPlus functions) || Version 0.9
*
...
* The plugin command 'Tint fade #777777 5' will fade the color from the current olor to the new, the last
...
* The arguments are optional (8=beamlength, 12=beamwidth, #FC961C=color, 3=beam density)
...
*
* This Plugin is a collaboration between the Killer Gin and Dairnon, you must credit us in your game credits page.
* This Plugin is a paid plugin. You must have purchased this plugin on STEAM or Itch.io to use.
*
...
($=> {
    const filter = $plugins.filter(function (p) {
        return p.description.contains('<AnisoftLightingSystem>') && p.status;
    })[0];
...
($ => {
    ...
    $.getLightData = function (data) {
        ...
        if (lightType === 'light' || lightType === 'fire' || lightType === 'flashlight') {
            ...
            if (lightType === 'flashlight') {
                ...
                bitmap = Anisoft.ALS.LightBitmapGenerator.getConeLight(length, width)
            } else {
                radius = notes.shift();
            }
            ...
            if (lightType) {
                ...
                data.lightData.push({
                    ...
                    tint: tint ? Anisoft.ALS.string2hex(tint) : 0xffffff,
                    bitmap: bitmap,
    ...
    $.makeSaveContents = function() {
        ...
        contents.lightData = Anisoft.ALS.LightManager.saveData();
        return contents;
    };
    ...
    $.extractSaveContents = function(contents){
        ...
        Anisoft.ALS.LightManager.loadData(contents.lightData);
    ...
    $.createGameObjects = function() {
        ...
        Anisoft.ALS.LightManager.reset(true);
    };

    if (Imported.YEP_EventMorpher) {
...
($=> {
    ...
    $.pluginCommand = function(command, args) {
        ...
        if (command === 'tint') {
            if (args[0] === 'set') {
                Anisoft.ALS.ambientLight = Anisoft.ALS.string2hex(args[1]);
                Anisoft.ALS.ambientLightTarget = Anisoft.ALS.ambientLight;
                Anisoft.ALS.ambientLightTime = 1;
            }
            if (args[0] === 'fade') {
                Anisoft.ALS.ambientLightTarget = Anisoft.ALS.string2hex(args[1])
                Anisoft.ALS.ambientLightTime = 0;
            }
        }
        ...
        if (command === 'light' || command === "fire" || command === "flashlight") {
            ...
            const player = args[0];
            if (player === 'radius') {
                const light = Anisoft.ALS.LightManager.getLight('player_0');
                ...
                if (/^#[0-9A-F]{6}$/i.test(tint)) {
                    light.tint = Anisoft.ALS.string2hex(tint)
            ...
            if (command === "flashlight") {
                ...
                if (args[0] === 'off' || args[0] === 'on') {
                    ...
                    let width = args[2];
                    if (args[0] === 'on') {
                        light.bitmap = Anisoft.ALS.LightBitmapGenerator.getConeLight(length !== undefined ? length : Anisoft.ALS.defaultFlashlightLength, width !== undefined ? width : Anisoft.ALS.defaultFlashlightWidth)
                    } else {
                        light.bitmap = null;
                    }
                    args.shift()
            ...
            if (player === 'radiusgrow') {
                ...
                const tint = args[2];
                if (/^#[0-9A-F]{6}$/i.test(tint)) {
                    light._tintTarget = Anisoft.ALS.string2hex(tint)
                }
            ...
            if (!isNaN(lastArg)) {
                ...
                if (event && event._lights) light = Anisoft.ALS.LightManager.getLight(event._lights[0]);
            }
            if (!light && !lightMultiple) return;

            if (args[0] === 'off' || args[0] === 'on') {
                if (lightMultiple) {
                    light = Anisoft.ALS.LightManager.rangeLights(Number(lastArg), {active: args[0] === 'on'})
                } else {
...
($ => {
    class Game_LightManager {
        ...
        createLight(event, data) {
            ...
            const light = new Anisoft.ALS.Game_Light(id, event._realX, event._realY, data);
        ...
        add(light, force) {
            ...
            if (Anisoft.ALS.lightMapContainer) {
                Anisoft.ALS.lightMapContainer.createSpriteLight(light);
            }
        }
        ...
        loadData(data) {
            ...
            for (let i = 0, len = data.length; i < len; ++i) {
                ...
                const light = new Anisoft.ALS.Game_Light(lightData.id, lightData.position.x, lightData.position.y, lightData);
                this.add(light, true);
            }
        }
...
($ => {
    class Game_Light {
        ...
        setDirty(value) {
            if (value) {
                    this._isDirty = true;
                    Anisoft.ALS.LightManager.addDirty(this);
            } else {
                this._isDirty = false;
                Anisoft.ALS.LightManager.removeDirty(this);
        ...
        refresh() {
            if (Anisoft.ALS.lightMapContainer) {
                const spriteLight = Anisoft.ALS.lightMapContainer._objLights[this._id]
...
($ => {
    class LightBitmapGenerator {
        ...
        createPointBitmap(key, resolution = Anisoft.ALS.defaultBitmapResolution) {
        ...
        getConeLight(coneLength, coneWidth, resolution = Anisoft.ALS.defaultBitmapResolution) {
...
($ => {
    class LightMapContainer extends PIXI.Container {
        ...
        createLightContainer () {
            this._lightMapContainer = Anisoft.ALS.lightMapContainer =(Anisoft.ALS.lightMapContainer ? Anisoft.ALS.lightMapContainer :  new Anisoft.ALS.LightMapContainer());
            Anisoft.ALS.lightMapContainer.reset();
            this.addChild(this._lightMapContainer);
        };

        createLights () {
            const lights = Anisoft.ALS.LightManager.currentMapLights();
        ...
        createShadowMap () {
            ...
            this._lightMapFilter = new Anisoft.ALS.LightmapFilter(Anisoft.ALS.ambientLight);
        ...
        createSpriteLight(light) {
            ...
            const spriteLight = new Anisoft.ALS.Sprite_Light(light);
...
($=> {
    $.moveLights = function() {
        ...
        for (let i = 0; i < this._lights.length; ++i) {
            ...
            const light = Anisoft.ALS.LightManager.getLight(id);
            if (!light) continue;
...
(function($) {
    ...
    $.performTransfer = function() {
        ...
        if ($gamePlayer.newMapId() !== $gameMap.mapId()) {
            console.log("reset?")
            Anisoft.ALS.LightManager.reset();
    ...
    $.setupLights = function() {
        const light = Anisoft.ALS.LightManager.createLight(this, {
            radius: Anisoft.ALS.playerRadius,
            tint: 0xf07829,
        });
    ...
    $.setupLights = function () {
        ...
        if (lightData) {
            for (let i = 0; i < lightData.length; ++i) {
                ...
                const light = Anisoft.ALS.LightManager.createLight(this, data);
...
($ => {
    ...
    $.update = function(sceneActive) {
        ...
        Anisoft.ALS.LightManager.update();
    };
...
($ => {
    ...
    $.updateAmbientLight = function() {
        let ambientLight = Anisoft.ALS.ambientLight;
        let ambientTarget = Anisoft.ALS.ambientLightTarget;
        let time = Anisoft.ALS.ambientLightTime;
        if (time <= 1) {
            time += Anisoft.ALS.ambientLightDuration;
            Anisoft.ALS.ambientLightTime = time;
            const color = Anisoft.ALS.lerpHexColor(ambientLight, ambientTarget, time)
            this._lightMapContainer._lightMapFilter.color = color;
        } else {
            Anisoft.ALS.ambientLight = ambientTarget;
        }
    }
...
($ => {
    ...
    $.render = function (stage) {
        ...
        if (this._skipCount === 0) {
             if (Anisoft.ALS.lightMapContainer)
                 Anisoft.ALS.lightMapContainer.render(SceneManager._scene._spriteset);
        }
